...
export '/backend/schema/util/mock_firestore.dart'
    show DocumentReference, SupabaseFirestore;
...
extension StringDocRef on String {
  DocumentReference get ref => SupabaseFirestore.instance.doc(this);
}
...
extension ListDivideExtDynamic on Iterable<dynamic> {
  Iterable<MapEntry<int, dynamic>> get enumerate => toList().asMap().entries;

  List<Widget> divide(Widget t, {bool Function(int)? filterFn}) => isEmpty
      ? []
      : (enumerate
          .map((e) => [e.value as Widget, if (filterFn == null || filterFn(e.key)) t])
          .expand((i) => i)
          .toList()
        ..removeLast());
}
